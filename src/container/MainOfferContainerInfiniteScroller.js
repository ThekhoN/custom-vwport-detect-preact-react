...
class MainOfferContainerInfiniteScroller extends Component {
  ...
  updateScrollY(){
    ...
    if(lastScrollYValue > currentScrollYValue) {
      this.setState({
        ...
        console.log('lastScroll: ', currentScrollYValue);
      });
  ...
  handleDisplayFilterControlOnScroll(){
    ...
    const { vwPortSize, offsetTop, scrollY, loadComplete, isLoading, forceLoading } = this.state;
    ...
    if(!loadComplete){
      if(lastScrollYValue > dodOffersOffsetTopStart - (vwPortSizeHeight/2)){
        // console.log('time to show das filter bar');
      ...
      console.log('this.state on loadComplete: ', this.state);
      console.log('this.offsetTop.dodOffersEnd: ', this.state.offsetTop.dodOffersEnd);
      console.log('lastScrollYValue: ', lastScrollYValue);
      console.log('dodOffersOffsetTopStart: ', dodOffersOffsetTopStart);
      if( lastScrollYValue < dodOffersOffsetTopStart || lastScrollYValue > (dodOffersOffsetTopEnd - vwPortSizeHeight) ){
        // console.log('showFilterControl...');
        this.hideFilterControl();
      }
      else {
        if(scrollDirection === 'top' && !isLoading && !forceLoading){
          this.showFilterControl();
        }
        else {
          this.hideFilterControl();
    ...
    // console.groupEnd('handleDisplayFilterControlOnScroll');
  }
  getDodOffsetTop(){
    ...
    this.setState({
      ...
      console.log('running getDodOffsetTop updated offsetTop  : ', this.state.offsetTop);
    });
  }
  detectElemInVwPort(){
  ...
  handleModalOpen(){
    ...
    // const scrollType = mobileView ? this.jumpToDodOffers : this.scrollToDodOffers;
    // scrollType();
  ...
  forceLoadAllOffers(){
    if(this.state.loadComplete){
      ...
      axios.get(forceLoadAllOffersURL)
      .then(response => {
        ...
        this.setState({
          ...
          console.log('forceLoad complete all offers. . .')
  ...
  componentDidMount(){
    ...
    window.addEventListener('scroll', debouncer(this.onScrollRunEvents, 60));
